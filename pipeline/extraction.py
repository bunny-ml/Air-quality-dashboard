...
def extract_data(args):
    ...
    for data_file_path in data_file_paths:
        ...
        query = compile_data_file_query(
            ...
            extract_query_template=execute_query_template
        )

        try:
            execute_query(con , query)
            logging.info(f"Extracying data from {data_file_path}")
        except IOException as e:
            logging.warning(f"Could not find data from {data_file_path}: {e}")
    close_database_connection(con)

def main():
    parser = argparse.ArgumentParser(description="CLI for Data Extraction")
    parser.add_argument(
        "--locations_file_path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help= "Path to SQL extraction query template"   
        )
    parser.add_argument(
        "--database_path",
        type=str,
